...
bricks_next_next = []
for i in range(4):
    bricks_next_next.append([0]*4)
# 儲存的方塊圖形陣列(4x4).
bricks_next_next_object = []
for i in range(4):
    bricks_next_next_object.append([0]*4)
...
def brickNew():
    ...
    if (lines > 0):
        ...
        game_mode = 1
        #本局分數
        point_max=lines_number*5
        point_rank[5]=point_max
...
def rank_array():
        global point_rank
        for y in range (5):
            if(point_rank[5-y]>point_rank[4-y]):
                rank_temp=point_rank[5-y]
                point_rank[5-y]=point_rank[4-y]
                point_rank[4-y]=rank_temp
#-------------------------------------------------------------------------
# 初始.
pygame.init()
# 顯示Title.
...
# 背景區塊.
background_bricks_next_next = Box(pygame, canvas, "background_bricks_next_next", [150, 50, 114, 114], color_gray)
#音效功能初始化
pg.mixer.init()
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            #刪除方塊(儲存的)
                 ...
                 brick_next_next_id = random.randint(1, 7)
                 brick_state = 0 #清空還在跑的方塊
                 #設定初始位置
                 container_x = 3
                 container_y =-4
    ...
    showFont( u"1.", 588, 420, color_block)
    showFont( str(int(point_rank[0])), 628, 420, color_block)
    showFont( u"2.", 588, 450, color_block)
    showFont( str(int(point_rank[1])), 628, 450, color_block)
    showFont( u"3.", 588, 480, color_block)
    showFont( str(int(point_rank[2])), 628, 480, color_block)
    showFont( u"4.", 588, 510, color_block)
    showFont( str(int(point_rank[3])), 628, 510, color_block)
    showFont( u"5.", 588, 540, color_block)
    showFont( str(int(point_rank[4])), 628, 540, color_block)
    
    #儲存的方塊
    showFont( u"儲存的方塊", 150, 16, color_block)
    # 顯示FPS.
    # 除錯訊息.
    if(debug_message):    
